...
class DNSaaSClientAction:
    ...
    def __init__(self, endpoint, tenant, token, maas_endpoint_address, maas_endpoint, dispose_maas):
        ...
        self.test_dns(endpoint,'')
        #self.__dnsaasclient = DNSaaSClientCore(endpoint, token)
    ...
    def update_record(self, domain_name, record_name, record_type, parameter_to_update, data, token):
        ...
        status, content = self.__dnsaasclient.do_request('PUT', '/records', json.dumps(msg_json),
                                                         token)
        return content
    ...
    def delete_record(self, domain_name, record_name, record_type, token):
        ...
        status, content = self.__dnsaasclient.do_request('DELETE', '/records',
                                                         json.dumps(json_record, sort_keys = False),
                                                         token)
    ...
    def init_dns(self):
        ...
        if DNSaaSClientAction.get_location() is None:
            response = requests.post(self.__endpoint + '/', headers = headers)
            env_sm = response.headers
            DNSaaSClientAction.set_location(str(env_sm.get('location')))
        else:
        ...
        for i in range(0, 20):
            ...
            if 'CREATE_COMPLETE' in response.content:
                attributes = re.findall('X-OCCI-Attribute: (.*)\"', response.text)

                for attr in attributes:
                    hh = re.split("=", attr)
                    print "Received Attributes " + hh[0]

                    if hh[0] == 'mcn.endpoint.api':
                        ip_dnsaas = re.sub('(.*)\"', '', hh[1])
                        print "API endpoint received " + ip_dnsaas
                        self.__urldnsaas = ip_dnsaas

                    if hh[0] == 'mcn.endpoint.forwarder':
                        ip_dnsaas = re.sub('(.*)\"', '', hh[1])
                ...
                if 'CREATE_FAILED' in response.content:
